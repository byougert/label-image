    ...
    def saveLabels(self, annotationFilePath):
        ...
        try:
            if self.labelFileFormat == LabelFileFormat.PASCAL_VOC:
                ...
                annotationFile_next_path = annotationFilePath.rstrip('.xml')[:-4:] + "%04d"%(int(annotationFilePath.rstrip('.xml')[-4:])+1) + '.xml'
                file_next_path = annotationFile_next_path.rstrip('.xml') + '.png'
                self.labelFile.savePascalVocFormat(annotationFilePath, shapes, self.filePath, self.imageData,
                                                   self.lineColor.getRgb(), self.fillColor.getRgb())
                if not os.path.exists(annotationFile_next_path):
                    try:
                        self.labelFile.savePascalVocFormat(annotationFile_next_path, shapes, file_next_path, self.imageData,
                                                  self.lineColor.getRgb(), self.fillColor.getRgb())
                    except:
                        pass
            elif self.labelFileFormat == LabelFileFormat.YOLO:
                ...
                annotationFile_next_path = annotationFilePath.rstrip('.txt')[:-6:] + "%06d"%(int(annotationFilePath.rstrip('.txt')[-6:])+1) + '.txt'
                file_next_path = annotationFile_next_path.rstrip('.txt') + '.png'
                self.labelFile.saveYoloFormat(annotationFilePath, shapes, self.filePath, self.imageData, self.labelHist,
                                              self.lineColor.getRgb(), self.fillColor.getRgb())
                if not os.path.exists(annotationFile_next_path):
                    try:
                        self.labelFile.saveYoloFormat(annotationFile_next_path, shapes, file_next_path, self.imageData, self.labelHist,
                                                  self.lineColor.getRgb(), self.fillColor.getRgb())
                    except:
                        pass
            elif self.labelFileFormat == LabelFileFormat.CREATE_ML:
                ...
                annotationFile_next_path = annotationFilePath.rstrip('.json')[:-6:] + "%06d"%(int(annotationFilePath.rstrip('.json')[-6:])+1) + '.json'
                file_next_path = annotationFile_next_path.rstrip('.json') + '.png'
                ...
                if not os.path.exists(annotationFile_next_path):
                    try:
                        self.labelFile.saveCreateMLFormat(annotationFile_next_path, shapes, file_next_path, self.imageData,
                                                  self.lineColor.getRgb(), self.fillColor.getRgb())
                    except:
                        pass
            else:
                no_back_path = annotationFilePath.rstrip('.xml')
                annotationFile_next_path = no_back_path[:-6:] + "%06d"%(int(no_back_path[-6:])+1) + '.xml'
                file_next_path = annotationFile_next_path.rstrip('.xml') + '.png'
                self.labelFile.save(annotationFilePath, shapes, self.filePath, self.imageData,
                                    self.lineColor.getRgb(), self.fillColor.getRgb())
                if not os.path.exists(annotationFile_next_path):
                    try:
                        self.labelFile.save(annotationFile_next_path, shapes, file_next_path, self.imageData,
                                                  self.lineColor.getRgb(), self.fillColor.getRgb())
                    except:
                        pass
